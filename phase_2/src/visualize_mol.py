"""
visualize_mol.py
----------------
Dwa wykresy:
  1. Profile energetyczne E(φ) dla każdego wiązania (MMFF94)
  2. Struktura 2D aspiryny jako SVG z podświetlonymi wiązaniami

Uruchomienie: uv run python src/visualize_mol.py
"""

import os
import sys

sys.path.insert(0, os.path.dirname(__file__))

import matplotlib.patches as mpatches
import matplotlib.pyplot as plt
import numpy as np
from rdkit import Chem
from rdkit.Chem import AllChem, rdDepictor, rdForceFieldHelpers, rdMolTransforms
from rdkit.Chem.Draw import rdMolDraw2D

from molecule import (
    build_dependency_graph,
    find_rotatable_bonds,
    greedy_graph_coloring,
    read_angles,
)

SMILES = "CC(=O)Oc1ccccc1C(=O)O"

# Kolory per kolor schedulera (0=niebieski, 1=zielony)
SCHED_COLORS = {
    0: "#3B8BD4",
    1: "#1D9E75",
    2: "#D85A30",
}

BOND_LABELS = {
    2: "acetyl C=O\n(ester)",
    3: "O→Ar\n(tlen estrowy)",
    9: "Ar→COOH\n(karboksyl)",
}


def get_bonds_with_coloring(smiles):
    mol = Chem.MolFromSmiles(smiles)
    mol_h = Chem.AddHs(mol)
    params = AllChem.ETKDGv3()
    params.randomSeed = 42
    AllChem.EmbedMolecule(mol_h, params)

    bonds = find_rotatable_bonds(mol_h)
    dep_graph = build_dependency_graph(bonds, mol_h)
    coloring = greedy_graph_coloring(dep_graph)
    read_angles(mol_h, bonds, conf_id=0)

    return mol, mol_h, bonds, coloring


def plot_energy_profiles(mol_h, bonds, coloring, output_path):
    """
    Dla każdego wiązania: skanuj φ od -180° do 180° co 5°,
    mierz MMFF94, rysuj profil energetyczny.
    Pozostałe wiązania zamrożone w aktualnej pozycji.
    """
    ff_props = rdForceFieldHelpers.MMFFGetMoleculeProperties(mol_h)
    ff = rdForceFieldHelpers.MMFFGetMoleculeForceField(mol_h, ff_props)
    conf = mol_h.GetConformer(0)

    n = len(bonds)
    fig, axes = plt.subplots(1, n, figsize=(5 * n, 4.5))
    if n == 1:
        axes = [axes]

    angles_range = np.linspace(-180, 175, 72)

    for ax, rb in zip(axes, bonds):
        color_idx = coloring.get(rb.bond_idx, 0)
        color = SCHED_COLORS.get(color_idx, "#888")

        # Zapisz aktualne kąty wszystkich wiązań
        saved = {}
        for b in bonds:
            saved[b.bond_idx] = rdMolTransforms.GetDihedralDeg(conf, *b.dihedral_atoms)

        energies = []
        for phi in angles_range:
            rdMolTransforms.SetDihedralDeg(conf, *rb.dihedral_atoms, phi)
            energies.append(ff.CalcEnergy())

        # Przywróć
        for b in bonds:
            rdMolTransforms.SetDihedralDeg(conf, *b.dihedral_atoms, saved[b.bond_idx])

        energies = np.array(energies)
        e_rel = energies - energies.min()

        ax.plot(angles_range, e_rel, color=color, lw=2.5, zorder=3)
        ax.fill_between(angles_range, e_rel, alpha=0.12, color=color)

        # Zaznacz aktualne minimum
        min_phi = angles_range[np.argmin(energies)]
        ax.axvline(
            min_phi,
            color="darkred",
            lw=1.5,
            ls="--",
            alpha=0.7,
            label=f"min: {min_phi:.0f}°",
        )

        # Zaznacz kT w 300K jako poziom bariery
        kT = 0.001987 * 300
        ax.axhline(
            kT, color="gray", lw=1, ls=":", alpha=0.6, label=f"kT (300K) = {kT:.2f}"
        )

        label = BOND_LABELS.get(rb.bond_idx, f"Bond {rb.bond_idx}")
        sched_label = f"Kolor schedulera: {color_idx}"
        ax.set_title(f"Bond {rb.bond_idx} — {label}\n{sched_label}", fontsize=10)
        ax.set_xlabel("Kąt φ (°)", fontsize=9)
        ax.set_ylabel("ΔE (kcal/mol)" if bonds.index(rb) == 0 else "", fontsize=9)
        ax.set_xlim(-180, 180)
        ax.set_xticks([-180, -90, 0, 90, 180])
        ax.legend(fontsize=8)
        ax.grid(True, alpha=0.2)

        # Adnotacja: ile minimów
        peaks_above_kT = np.sum(np.diff(np.sign(np.diff(e_rel))) < 0)
        ax.text(
            0.02,
            0.97,
            f"Lokalne min: ~{peaks_above_kT + 1}",
            transform=ax.transAxes,
            fontsize=8,
            va="top",
            color="gray",
        )

    fig.suptitle(
        "Profile energetyczne MMFF94 — Aspiryna\n"
        "E(φ) przy zamrożonych pozostałych wiązaniach",
        fontsize=12,
        y=1.02,
    )
    plt.tight_layout()
    plt.savefig(output_path, dpi=150, bbox_inches="tight")
    print(f"Profile energetyczne: {output_path}")
    plt.close()


def draw_structure_svg(mol, bonds, coloring, output_path):
    """
    2D struktura jako SVG z podświetlonymi wiązaniami.
    Kolor = kolor schedulera (co może się aktualizować równolegle).
    """
    rdDepictor.Compute2DCoords(mol)

    drawer = rdMolDraw2D.MolDraw2DSVG(600, 380)
    drawer.drawOptions().addAtomIndices = False
    drawer.drawOptions().addStereoAnnotation = False
    drawer.drawOptions().padding = 0.15

    highlight_bonds = []
    bond_colors = {}
    highlight_atoms = []
    atom_colors = {}

    for rb in bonds:
        bond = mol.GetBondBetweenAtoms(rb.atom_i, rb.atom_j)
        if bond is None:
            continue
        bidx = bond.GetIdx()
        highlight_bonds.append(bidx)

        color_idx = coloring.get(rb.bond_idx, 0)
        hex_color = SCHED_COLORS.get(color_idx, "#888888")
        # RDKit wymaga tuple (r, g, b) w [0,1]
        r = int(hex_color[1:3], 16) / 255
        g = int(hex_color[3:5], 16) / 255
        b = int(hex_color[5:7], 16) / 255
        bond_colors[bidx] = (r, g, b)

        for atom_idx in [rb.atom_i, rb.atom_j]:
            if atom_idx not in highlight_atoms:
                highlight_atoms.append(atom_idx)
            atom_colors[atom_idx] = (r, g, b)

    drawer.DrawMolecule(
        mol,
        highlightAtoms=highlight_atoms,
        highlightAtomColors=atom_colors,
        highlightBonds=highlight_bonds,
        highlightBondColors=bond_colors,
    )
    drawer.FinishDrawing()
    svg_text = drawer.GetDrawingText()

    with open(output_path, "w") as f:
        f.write(svg_text)
    print(f"Struktura 2D (SVG): {output_path}")
    return svg_text


def print_summary(bonds, coloring, dep_graph):
    print("\n=== Podsumowanie grafu wiązań ===")
    for b in bonds:
        c = coloring.get(b.bond_idx, "?")
        nbrs = dep_graph.get(b.bond_idx, set())
        label = BOND_LABELS.get(b.bond_idx, "")
        print(
            f"  Bond {b.bond_idx:2d}  [{label:20s}]  "
            f"kolor={c}  φ={b.current_angle:7.1f}°  "
            f"sąsiedzi={sorted(nbrs)}"
        )

    print("\n=== Interpretacja schedulera ===")
    from collections import defaultdict

    groups = defaultdict(list)
    for bidx, c in coloring.items():
        groups[c].append(bidx)
    for c in sorted(groups):
        print(f"  Krok {c}: aktualizuj równolegle wiązania {sorted(groups[c])}")


if __name__ == "__main__":
    os.makedirs("results", exist_ok=True)

    print("Przygotowuję cząsteczkę...")
    mol, mol_h, bonds, coloring = get_bonds_with_coloring(SMILES)
    dep_graph = build_dependency_graph(bonds, mol_h)

    print_summary(bonds, coloring, dep_graph)

    print("\nGeneruję profile energetyczne...")
    plot_energy_profiles(mol_h, bonds, coloring, "results/aspirin_energy_profiles.png")

    print("Generuję strukturę 2D...")
    draw_structure_svg(mol, bonds, coloring, "results/aspirin_structure.svg")

    print("\nGotowe! Sprawdź results/")
    print("  aspirin_energy_profiles.png — dlaczego Bond 2/9 skaczą")
    print("  aspirin_structure.svg       — otwórz w przeglądarce")
