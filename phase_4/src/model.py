...
class MoleculeModel(Model):
    ...
    def __init__(
        ...
        # strategy: str (all bonds same) or list[str] (one per bond in detection order)
        self._strategy_input = strategy
        self.init = init
        self.comm_cutoff = comm_cutoff
        ...
        # strategy may be a str (all same) or a list[str] (per-bond)
        # ------------------------------------------------------------------
        n_bonds = len(self.bonds)
        if isinstance(self._strategy_input, str):
            strategy_list = [self._strategy_input] * n_bonds
        else:
            strategy_list = list(self._strategy_input)
            if len(strategy_list) != n_bonds:
                raise ValueError(
                    f"strategy list length {len(strategy_list)} != n_bonds {n_bonds}"
                )

        # Canonical label for this model
        from collections import Counter
        counts = Counter(strategy_list)
        if len(counts) == 1:
            self.strategy: str = strategy_list[0]
        else:
            self.strategy = "+".join(
                f"{v}x{k}" for k, v in sorted(counts.items())
            )
        self._strategy_list: List[str] = strategy_list

        self.bond_agents: List[BondAgent] = []
        for bond, strat_name in zip(self.bonds, strategy_list):
            agent = AGENT_CLASSES[strat_name](self, bond, self.master_mol, temperature)
    ...
    def summary(self) -> dict:
        ...
        # Per-strategy acceptance rates for heterogeneous populations
        from collections import defaultdict
        strat_acc: dict = defaultdict(list)
        for agent in self.bond_agents:
            strat_acc[agent.strategy].append(agent.acceptance_rate())
        per_strategy_acceptance = {
            k: float(np.mean(v)) for k, v in strat_acc.items()
        }
        ...
        return {
            ...
            "per_strategy_acceptance": per_strategy_acceptance,
            "n_bonds": len(self.bonds),
        }
