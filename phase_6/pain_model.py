...
class PainModel(Model):
    ...
    def fork_consensus(
        self,
        consensus_angles: np.ndarray,
        is_stable: np.ndarray,
        sigma_unstable: float = 45.0,
        seed: int | None = None,
    ) -> "PainModel":
        """
        Per-bond consensus offspring.

        For bonds where the elite agrees (is_stable[b] == True):
            transplant consensus_angles[b] directly  → exploitation
        For bonds where the elite is dispersed (is_stable[b] == False):
            keep this individual's current angle + Gaussian mutation  → exploration
        """
        rng = np.random.default_rng(seed)
        child_seed = int(rng.integers(0, 2**31))
        child = PainModel(self.smiles, self.params, init="zeros",
                          n_steps=0, seed=child_seed)
        conf = child.mol.GetConformer(0)

        for b_idx, (b_c, b_p) in enumerate(zip(child.bonds, self.bonds)):
            a, i, j, bk = b_c.dihedral_atoms
            if is_stable[b_idx]:
                new_phi = float(consensus_angles[b_idx])
            else:
                delta = float(rng.normal(0.0, sigma_unstable))
                new_phi = b_p.current_angle + delta
            new_phi = ((new_phi + 180.0) % 360.0) - 180.0
            rdMolTransforms.SetDihedralDeg(conf, a, i, j, bk, new_phi)
            b_c.current_angle = new_phi

        child.energy_snapshots = [child._mmff_energy()]
        child.pain_snapshots   = [child._total_pain()]
        child.n_rotations      = 0
    ...
    def fork(
        ...
        """
        Return a new PainModel that starts from the current conformation.

        Optionally mutates `perturb_bonds` randomly chosen dihedrals by
        Gaussian(0, sigma) degrees — the evolutionary mutation operator.
        """
        rng = np.random.default_rng(seed)
        child_seed = int(rng.integers(0, 2**31))
        child = PainModel(self.smiles, self.params, init="zeros",
        ...
        # Copy parent angles
        for b_c, b_p in zip(child.bonds, self.bonds):
            a, i, j, bk = b_c.dihedral_atoms
            rdMolTransforms.SetDihedralDeg(conf, a, i, j, bk, b_p.current_angle)
            b_c.current_angle = b_p.current_angle

        # Mutate a random subset of bonds
        if perturb_bonds > 0:
            n = min(perturb_bonds, len(child.bonds))
            idxs = rng.choice(len(child.bonds), size=n, replace=False)
            for idx in idxs:
                b = child.bonds[idx]
                a, i, j, bk = b.dihedral_atoms
                delta = float(rng.normal(0.0, sigma))
                new_phi = ((b.current_angle + delta + 180.0) % 360.0) - 180.0
                rdMolTransforms.SetDihedralDeg(conf, a, i, j, bk, new_phi)
                b.current_angle = new_phi

        # Reset snapshots to reflect the actual starting conformation
        child.energy_snapshots = [child._mmff_energy()]
        child.pain_snapshots   = [child._total_pain()]
        child.n_rotations      = 0
        child.current_step     = 0
